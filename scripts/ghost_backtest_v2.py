...
USAGE (synthetic data — no licence, no network)
───────────────────────────────────────────────
    python scripts/make_synthetic.py
    python scripts/ghost_backtest_v2.py --synthetic

  --synthetic reads data/synthetic/ghost_scores_fq.parquet and data/synthetic/prices.parquet
  (or --data_dir) instead of downloading prices, uses the made-up benchmark SYNMKT and the
  date window in the folder's manifest.json. The numbers it prints say nothing about ghost
  postings; they show that the pipeline runs.
...
# plt.show() opens a blocking window outside a notebook; the command line turns it off (--show turns it back on)
SHOW_PLOTS = True


def _yfinance():
    """yfinance is only needed for real prices; --synthetic runs without it."""
    try:
        import yfinance as yf
    except ModuleNotFoundError as err:
        raise SystemExit(
            "yfinance is not installed. Install the full requirements (pip install -r requirements.txt), "
            "or run on synthetic data with --synthetic, which needs no network."
        ) from err
    return yf


def benchmark_label(benchmark_ticker: str) -> str:
    return f"Buy & Hold ({benchmark_ticker})"


def style_key(col: str) -> str:
    """PALETTE and DASHES are keyed by the SPY label; any benchmark takes the benchmark style."""
    return STRATEGY_LABELS["benchmark"] if col.startswith("Buy & Hold (") else col

# ─────────────────────────────────────────────────────────────────────────────
# DEFAULT CONFIGURATION  (override via CLI args or main() kwargs)
# ─────────────────────────────────────────────────────────────────────────────

PANDAS_FREQ = {"Q": "QE", "M": "ME", "W": "W"}   # the README's short names -> pandas >= 2.2 offset aliases
...
DEFAULTS = dict(
    ...
    rebal_freq            = "Q",           # 'Q' quarterly | 'M' monthly (translated by PANDAS_FREQ below)
...
def download_prices(tickers:             list,
                    ...
                    identity_check:      bool,
                    output_dir:          Path,
                    synthetic_dir:       str | None = None) -> pd.DataFrame:
    """
    ...
    if synthetic_dir:
        path = Path(synthetic_dir) / "prices.parquet"
        print(f"  Reading synthetic prices for {len(all_tickers)} tickers from {path} (no download) ...")
        if not path.exists():
            raise FileNotFoundError(f"Synthetic prices not found: {path}\nRun: python scripts/make_synthetic.py")
        long = pd.read_parquet(path)
        long = long[long["ticker"].isin(all_tickers)
                    & (long["date"] >= pd.Timestamp(start)) & (long["date"] <= pd.Timestamp(end))]
        prices  = long.pivot(index="date", columns="ticker", values="close").sort_index()
        volumes = long.pivot(index="date", columns="ticker", values="volume").sort_index()
        identity_check = False          # made-up tickers: nothing to compare against Yahoo Finance
    else:
        yf = _yfinance()
...
def run_backtest(ghost_df:         pd.DataFrame,
    ...
    # pandas renamed the period-end offsets in 2.2 and removed the old spellings in 3.0 ("Q" -> "QE", "M" -> "ME").
    # The command line keeps the short names the README documents; they are translated here, so the same command runs
    # on pandas 2.2 and on 3.x. (A fresh install of requirements-synthetic.txt brings 3.x, where "Q" raises
    # ValueError: Invalid frequency: Q — this machine happened to have 2.3 installed, so it only showed in a clean venv.)
    rebal_raw = pd.date_range(start=start_date, end=end_date, freq=PANDAS_FREQ.get(rebal_freq, rebal_freq))
    ...
    nav.columns = [benchmark_label(benchmark_ticker) if s == "benchmark" else STRATEGY_LABELS[s]
                   for s in strategies]
...
def plot_results(nav: pd.DataFrame,
    ...
    for col in nav.columns:
        c = PALETTE.get(style_key(col), "#555")
        d = DASHES.get(style_key(col), (1, 0))
        lw = 1.5 if col.startswith("Buy & Hold (") else 2.2
    ...
    for col in nav.columns:
        ...
        ax3.plot(nav.index, rs, color=c, linewidth=1.8, label=col,
                 linestyle=(0, DASHES.get(style_key(col), (1, 0))))
...
def main(ghost_file:           str   = DEFAULTS["ghost_file"],
    ...
    print(f"  Identity check    : {'on' if identity_check and not synthetic_dir else 'off'}")
    if synthetic_dir:
        print(f"  Data              : SYNTHETIC ({synthetic_dir}) — made-up tickers and prices")
    print("═" * 62)
    ...
    # Step 2 — Prices
    print("\n[2 / 5]  " + ("Reading synthetic prices" if synthetic_dir else "Downloading historical prices")
...
def resolve_inputs(args) -> dict:
    """Fill in what --synthetic and --data_dir imply, without overriding anything given explicitly."""
    import json
    data_dir = args.data_dir or ("data/synthetic" if args.synthetic else None)
    ghost_file = args.ghost_file or (str(Path(data_dir) / "ghost_scores_fq.parquet") if data_dir else DEFAULTS["ghost_file"])
    start, end, benchmark = args.start_date, args.end_date, args.benchmark
    if args.synthetic:
        manifest_path = Path(data_dir) / "manifest.json"
        if not manifest_path.exists():
            raise SystemExit(f"No synthetic data in {data_dir}/ (manifest.json missing). Run: python scripts/make_synthetic.py")
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        start = start or manifest["backtest"][0]
        end = end or manifest["backtest"][1]
        benchmark = benchmark or manifest["benchmark"]
    return dict(ghost_file=ghost_file, start_date=start or DEFAULTS["start_date"], end_date=end or DEFAULTS["end_date"],
                benchmark_ticker=benchmark or DEFAULTS["benchmark_ticker"], synthetic_dir=data_dir if args.synthetic else None)


def add_common_arguments(parser: argparse.ArgumentParser) -> None:
    """Arguments both scripts share, with one name each (the older spelling --vol_filter still works)."""
    parser.add_argument("--synthetic",            action="store_true",
                        help="Run on the synthetic files from scripts/make_synthetic.py: no download, no licensed data")
    parser.add_argument("--data_dir", "--data-dir", default=None,
                        help="Folder with ghost_scores_fq.parquet (and prices.parquet with --synthetic); "
                             "default data/synthetic with --synthetic")
    parser.add_argument("--ghost_file",           default=None,
                        help=f"Ghost score file (default: {DEFAULTS['ghost_file']}, or inside --data_dir)")
    parser.add_argument("--start_date",           default=None,
                        help=f"Backtest start (default: {DEFAULTS['start_date']}; with --synthetic, the manifest's window)")
    parser.add_argument("--end_date",             default=None,
                        help="Backtest end (default: today; with --synthetic, the manifest's window)")
    parser.add_argument("--lag_days",             type=int,   default=DEFAULTS["lag_days"],
                        help="Days after quarter-end before a quarter's scores may be used")
    ...
    parser.add_argument("--benchmark",            default=None,
                        help=f"Benchmark ticker (default: {DEFAULTS['benchmark_ticker']}; with --synthetic, SYNMKT)")
    ...
    parser.add_argument("--vol_filter_threshold", "--vol_filter", dest="vol_filter_threshold",
                        type=float, default=DEFAULTS["vol_filter_threshold"],
    ...
    parser.add_argument("--no_identity_check",    action="store_true",
                        help="Disable ticker identity cross-check (always off with --synthetic)")
    parser.add_argument("--output_dir",           default=DEFAULTS["output_dir"])
    parser.add_argument("--show",                 action="store_true",
                        help="Open the chart window after saving it (off by default on the command line)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--concentration",        type=float, default=DEFAULTS["concentration"],
                        help="Fraction of the scored universe in each leg")

    args = parser.parse_args()
    SHOW_PLOTS = args.show
    if not SHOW_PLOTS:
        plt.switch_backend("Agg")   # no window will open, so no display is needed (servers, CI, a terminal over SSH)
    inputs = resolve_inputs(args)

    nav, metrics = main(
        ghost_file           = inputs["ghost_file"],
        ...
        synthetic_dir        = inputs["synthetic_dir"],
    )
