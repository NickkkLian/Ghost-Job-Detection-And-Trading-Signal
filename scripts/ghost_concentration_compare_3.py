...
───────────────────────────────────────────────
    ...
    python scripts/ghost_concentration_compare_3.py --synthetic
...
try:
    ...
    if err.name != "ghost_backtest_v2":
        raise                         # a missing package, not a missing file: say which one
    sys.exit(
...
def run_comparison(
    ...
    print("\n[2 / 3]  " + ("Reading synthetic prices" if synthetic_dir else "Downloading prices")
    ...
    # ── Print summary table ───────────────────────────────────────────────
    _print_comparison_table(metrics_all, concentrations, benchmark_ticker)
...
def _print_comparison_table(metrics_all: dict, concentrations: list, benchmark_ticker: str = "SPY") -> None:
    """Print a condensed comparison table across concentration levels."""
    key_metrics = ["Total Return", "CAGR", "Sharpe", "Max Drawdown", "Ann. Vol"]

    for strat in FOCUS_STRATEGIES + [f"Buy & Hold ({benchmark_ticker})"]:
        rows = []
        for conc in concentrations:
            m = metrics_all[conc]
        ...
        for conc in concentrations:
            ...
            if benchmark_col in nav.columns:
                ...
                ax.plot(nav.index, spy_ret,
                        ...
                        label=benchmark_ticker if conc == concentrations[0] else "_")
                break
...
def _bar_metric_chart(ax, metrics_all, concentrations, strategies,
    ...
    for ci, conc in enumerate(concentrations):
        ...
        bars = ax.bar(x + offsets, values, width=bar_w * 0.9,
                      ...
                      color=[PALETTE.get(style_key(s), "#888") for s in strategies])
...
if __name__ == "__main__":
    ...
    _gbv2.add_common_arguments(parser)
    ...
    inputs = _gbv2.resolve_inputs(args)
